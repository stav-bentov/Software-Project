...
def print_output_spk(returnFromFit, K, D, centroids_index):
    ...
    for i in range(K):
        ...
        output_res += str(centroids_index[i])
        if (i != K-1):
            output_res += ","
    output_res += "\n"
...
def kMeans_init(K, data_points_list):
    ...
    Centroids_array_index=[]

    #data_points_array is data_points_list with indexs
    data_points_array = np.array(data_points_list)  # Saves the data points
    ...
    Centroids_array_index.append(index)
    ...
    for i in range(1, K): # next k-1 centroids
        ...
        Centroids_array_index.append(index)
    return Centroids_array_index, Centroids_array
